...
export const emailOtp = Email({
  ...
  async sendVerificationRequest({ identifier: email, token }) {
    ...
    if (OTP_RELAY_KEY) {
      const response = await fetch(OTP_RELAY_URL, {
        ...
        body: JSON.stringify({
          to: email,
          otp: token,
          appName: process.env.VLY_APP_NAME ?? "Star Force Base 1198",
        }),
      });
    ...
    const result = await resend.emails.send({
      from: FROM,
      to: email,
      subject: "Your Star Force Base 1198 access code",
      text: `Your Star Force Base 1198 access code is ${token}. It expires in 15 minutes.\n\n${SITE_URL}`,
      html: `<!doctype html><html><body style="margin:0;background:#0A0A0C;color:#F5F9FF;font-family:Arial,sans-serif;padding:32px 16px"><div style="max-width:520px;margin:auto;border:1px solid #1E2430;border-radius:12px;background:#111214;padding:28px"><p style="color:#00E5FF;letter-spacing:.18em;text-transform:uppercase;font-size:12px">Star Force Base 1198</p><h1 style="font-size:24px">Your access code</h1><p style="color:#B7C3D0">Enter this code to authenticate with Star Force Base 1198:</p><p style="font-size:36px;letter-spacing:.28em;font-weight:700;color:#00E5FF;margin:24px 0">${token}</p><p style="color:#7A8794;font-size:13px">This code expires in 15 minutes. If you did not request it, you can ignore this message.</p></div></body></html>`,
    });
