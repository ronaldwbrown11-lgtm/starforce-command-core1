...
## Sep 9 — Auto-regenerating source package (current session)

- **Root cause fixed:** `ship to site.bat` previously downloaded
  `public/starforce-source-latest.tar.gz`, which was only regenerated by
  hand — so deploys either 404'd or shipped an old build (the "nothing to
  push" / stale-site incident). The package is now rebuilt automatically on
  every `bun run build` / `build:ci` via **`scripts/package-source.ts`**
  (also runnable standalone: `bun run package:source`).
- Every package carries a **`BUILD-INFO.txt`** stamp (build time, commit,
  file count); the bat reads it, refuses unstamped packages, and puts it in
  the commit message so the Actions log shows exactly which build shipped.
- The packer **self-verifies** (tar listing checked against the file walk)
  and hard-fails if required files are missing or secrets (`.env*`) would be
  packed. Exclusions live in `sf-excludes.txt`.
- **Fixed exclusion bugs** that had been dropping site-critical files
  (`.htaccess`, `sw.js`, `manifest.webmanifest`, `logo.svg`) from source
  packages, and stopped packing old archives/backups (package shrank from
  12.5 MB to 5.2 MB). Archives are purged from `dist/` after each build and
  excluded from the Hostinger FTP upload in both deploy workflows.
- `ship to site.bat` rewritten: distinct, plain-language results for fresh
  code / 404 / corrupt package / nothing-to-push, and a stamped commit.
...
| Sep 9 | current source + auto-packaging (`bun run package:source`) |
